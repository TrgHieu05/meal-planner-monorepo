...
| Dịch vụ | Vai trò chính | Điểm mạnh | Điểm yếu | Mức phù hợp với repo này |
| --- | --- | --- | --- | --- |
| `Neon` | PostgreSQL managed cho app | Nhẹ, nhanh, DX tốt, dễ tạo `staging` branch và reset theo `production` khi cần | Một số nhu cầu network/private infra sẽ hạn chế hơn RDS | Rất phù hợp cho giai đoạn đầu |
...
Giai đoạn đầu nên chọn `Neon` với mô hình `1 project` và `2 branch` dài hạn là `production` và `staging`. Chỉ nên tách thành `2 project` riêng khi cần mức cô lập vận hành hoặc compliance cao hơn. Nếu sau này backend chuyển lên AWS và cần private VPC/networking đầy đủ, cân nhắc chuyển sang `RDS`.
...
| Database | `Neon Postgres` | Phù hợp Prisma/Postgres, ít vận hành và hỗ trợ tách `production`/`staging` bằng branch rõ ràng |
