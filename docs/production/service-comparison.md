...
- tách môi trường staging và production bằng branch hoặc project độc lập
...
| Dịch vụ | Vai trò chính | Điểm mạnh | Điểm yếu | Mức phù hợp với repo này |
| --- | --- | --- | --- | --- |
| `Neon` | PostgreSQL managed cho app | Nhẹ, nhanh, DX tốt, hỗ trợ preview branch `schema-only` theo Pull Request và bootstrap dữ liệu test/sanitized | Một số nhu cầu network/private infra sẽ hạn chế hơn RDS | Rất phù hợp cho giai đoạn đầu |
...
Giai đoạn đầu nên chọn `Neon` với mô hình `1 project`, giữ `production` branch làm gốc và tạo preview branch `schema-only` theo từng Pull Request. Chỉ nên tách thành `2 project` riêng khi cần mức cô lập vận hành hoặc compliance cao hơn. Nếu sau này backend chuyển lên AWS và cần private VPC/networking đầy đủ, cân nhắc chuyển sang `RDS`.
...
| Database | `Neon Postgres` | Phù hợp Prisma/Postgres, ít vận hành và hỗ trợ preview branch theo Pull Request rất tốt |
