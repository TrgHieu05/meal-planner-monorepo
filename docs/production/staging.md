...
2. Merge vào `main` sau khi Pull Request pass.
3. CI/CD deploy commit trên `main` lên `staging`.
4. QA kiểm tra trên `staging`.
5. Sau approval, cùng commit đó mới được deploy lên `production`.
...
- 1 database staging riêng, không dùng chung với production
...
- dùng PostgreSQL staging riêng
- không dùng chung database với production
