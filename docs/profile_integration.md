...
- Phần còn lại tập trung vào manual QA, kiểm thử runtime trên thiết bị và xác nhận end-to-end trên môi trường thật.

## Blocker ưu tiên cao

### 1. Thống nhất `gender` domain

- [x] Thống nhất domain của `gender` giữa database, backend, shared schema và mobile UI.
- [x] Chốt rõ hệ thống có cho phép giá trị `U` / `UNKNOWN` / `Prefer not to say` hay không. `[Cần bạn làm rõ]` **Không cho phép các giá trị ngoài `M | F`. Trước onboarding, `user.gender` phải nullable ở database/shared schema/backend; giá trị này sẽ được cập nhật qua luồng onboarding.**
- [x] Nếu chỉ cho phép `M | F`, cập nhật luồng tạo user Google để không sinh giá trị ngoài schema.
- [x] Nếu cho phép thêm giá trị mới, cập nhật đồng bộ Prisma schema, shared schema, backend validation và mobile mapping. Không áp dụng trong v1 vì `gender` đã được khóa về `M | F | null`.
- [x] Bổ sung test cho trường hợp user mới đăng nhập Google rồi gọi `GET /api/v1/users/me` và `GET /api/v1/profile/overview`.

### 2. Chốt chiến lược tạo `profile` cho user mới

- [x] Quyết định `profile` được tạo tự động ngay sau login hay chỉ tạo khi user hoàn thành onboarding. `[Cần bạn làm rõ]` **Chỉ tạo sau khi onboarding. Nhưng lưu ý rằng trong luồng onboarding thì có phần cập nhật thông tin của user (gender, bỉthday)**
- [x] Nếu chọn auto-bootstrap, định nghĩa default values hợp lệ cho `dietTypeId`, `goalId`, `cuisineTypeId`, `targetCalories`, `activityLevel`. `[Cần bạn làm rõ]` **Không áp dụng trong v1 vì profile không auto-bootstrap; profile chỉ được tạo sau onboarding.**
- [x] Nếu chọn onboarding-driven bootstrap, bổ sung endpoint tạo profile ban đầu, ví dụ `POST /api/v1/profile`.
- [x] Đảm bảo user mới không bị `404 Profile not found for the current user` khi đi qua luồng chuẩn của app.
- [x] Bổ sung test cho first-time user theo chiến lược đã chọn.

### 3. Bổ sung nguồn dữ liệu ingredient cho mobile

- [x] Thiết kế API ingredient catalog/search để mobile chọn allergy và favorite ingredient.
- [x] Chốt endpoint sẽ là danh sách toàn bộ, search endpoint, hay cả hai. `[Cần bạn làm rõ]` **Cả hai, nhưng triển khai 1 endpoint duy nhất: Khi q rỗng: trả danh sách browse mặc định. Khi q có giá trị: trả kết quả search.**
- [x] Chốt có cần pagination / debounced search / lọc theo tên hay không. `[Cần bạn làm rõ]` **Có pagniation với 30 nguyên liệu 1 lần gọi và search theo tên, debounce 500ms, không có bộ lọc**
- [x] Định nghĩa response tối thiểu cho selector: `id`, `name`.
- [x] Tài liệu hóa contract lỗi `409` khi ingredient bị conflict giữa allergy và favorite ingredient.

### 4. Chốt các field thực sự thuộc scope `profile`

- [x] Xác nhận `cuisineType` là single-select hay multi-select. `[Cần bạn làm rõ]`**Hiện tại chỉ cho phép single-select để đơn giản hóa schema và UI**
- [x] Nếu multi-select là requirement thật, cập nhật DB schema, API contract và mobile types cho phù hợp. Không áp dụng trong v1 vì `cuisineType` đã được chốt là single-select.
- [x] Nếu single-select là requirement thật, cập nhật mobile types và UI để không còn mô hình `cuisineTypes: string[]`.
- [x] Xác nhận `notificationsEnabled` có thuộc scope profile v1 hay chưa. `[Cần bạn làm rõ]` **Không thuộc scope v1, loại bỏ nếu tồn tại**
- [x] Xác nhận `bodyFatPercent` có thuộc scope metric/profile v1 hay chưa. `[Cần bạn làm rõ]` **Không thuộc scope v1, loại bỏ nếu tồn tại**
...
- [x] Cập nhật seed data để có dữ liệu kiểm thử phù hợp cho profile flow.
- [x] Nếu thêm ingredient catalog endpoint có search, xem lại index hoặc unique constraint cần thiết cho bảng `Ingredient`.
...
- [X] Đăng nhập bằng Google với user mới hoàn toàn.
- [X] Xác minh app điều hướng đúng theo flow đã chốt.
- [X] Xác minh xem profile screen có load dữ liệu thật không.
...
- [x] Nếu thay đổi env vars hoặc flow mobile, cập nhật `.env.example`.
...
- [x] `gender` cuối cùng là `M/F` hay cần thêm `Unknown/Other/Prefer not to say`? `[Cần bạn làm rõ]` **Chốt: chỉ cho phép `M | F`; trước onboarding thì `user.gender` phải nullable.**
- [x] `profile` có được tạo tự động khi user login lần đầu không? `[Cần bạn làm rõ]` **Chốt: không auto-create; chỉ tạo sau khi hoàn thành onboarding.**
- [x] Nếu auto-create profile, default values hợp lệ cho các foreign key là gì? `[Cần bạn làm rõ]` **Chốt: không áp dụng trong v1 vì không dùng auto-bootstrap profile.**
- [x] `cuisineType` là single-select hay multi-select? `[Cần bạn làm rõ]` **Chốt: single-select.**
- [x] `notificationsEnabled` có nằm trong scope feature profile hiện tại không? `[Cần bạn làm rõ]` **Chốt: không thuộc scope v1.**
- [x] `bodyFatPercent` có nằm trong scope metric/profile hiện tại không? `[Cần bạn làm rõ]` **Chốt: không thuộc scope v1.**
- [x] Ingredient selector cần full list, search-only, hay list + search? `[Cần bạn làm rõ]` **Chốt: dùng 1 endpoint duy nhất hỗ trợ cả browse mặc định và search theo tên.**
- [x] Contract của ingredient endpoint sẽ được khóa như thế nào? `[Cần bạn làm rõ]` **Chốt: `GET /api/v1/ingredients?q=&page=1&pageSize=30` trả response dạng `{ items, page, pageSize, total, hasMore }`, mỗi item tối thiểu gồm `{ id, name }`.**
- [x] Backend có trả tín hiệu explicit cho onboarding status hay để mobile tự suy luận? `[Cần bạn làm rõ]` **Chốt: backend trả explicit field `isOnboardingCompleted`; mobile route guard dùng field này làm nguồn sự thật.**
- [x] Sau login lần đầu, user nên vào onboarding bắt buộc hay vào thẳng app? `[Cần bạn làm rõ]` **Chốt: bắt buộc hoàn thiện onboarding trước.**
- [x] Khi profile chưa hoàn chỉnh, app có chặn các feature khác hay không? `[Cần bạn làm rõ]` **Chốt: chặn toàn bộ tabs cho đến khi server xác nhận onboarding complete.**
- [x] Mobile có cần interceptor tự refresh/retry khi gặp `401` trong phạm vi task này không? `[Cần bạn làm rõ]` **Chốt: có.**
...
- [x] Profile screen trên mobile hiển thị dữ liệu thật từ backend.
- [x] User có thể cập nhật basic info, preferences, metric, allergy và favorite ingredient từ mobile.
- [ ] Dữ liệu sau khi cập nhật vẫn đúng sau khi restart app.
- [x] Các lỗi validation/conflict được hiển thị rõ ràng cho người dùng.
- [x] Backend và mobile đều có test đủ để bảo vệ contract chính.
- [x] Tài liệu và OpenAPI được cập nhật theo implementation cuối cùng.